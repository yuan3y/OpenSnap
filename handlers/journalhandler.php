...
class UsersHandler {
	//-------------------------------------------------------------------------------------------------------------------
	//get journal  base on journal id  - for user edit , maybe need 2 condition?
	...
	//-------------------------------------------------------------------------------------------------------------------
	//get journal  base on user id   - own journal feed

	function get($user_id) {//get full product details when productid or barcode is present
		$sql = "SELECT `entry_id`,`user_id`,`product_id`,`timestamp`,`image`,`rating_ease`,`rating_safety`,`rating_reseal`,`rating_overall`,`comment`FROM `journal` WHERE `entry_id`='$user_id'";
		if ($result = mysqli_query($GLOBALS['con'], $sql)) {
			echo _response(mysqli_fetch_all($result,MYSQLI_ASSOC)[0]);
			mysqli_free_result($result);
		}
	...
	//-------------------------------------------------------------------------------------------------------------------
	//get journal  base on product id  - broswe feed or company such enquires

		function get($product_id) {//get full product details when productid or barcode is present
		$sql = "SELECT `entry_id`,`user_id`,`product_id`,`timestamp`,`image`,`rating_ease`,`rating_safety`,`rating_reseal`,`rating_overall`,`comment`FROM `journal` WHERE `entry_id`='$product_id'";
		if ($result = mysqli_query($GLOBALS['con'], $sql)) {
			echo _response(mysqli_fetch_all($result,MYSQLI_ASSOC)[0]);
			mysqli_free_result($result);
		}
